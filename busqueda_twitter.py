...
for each_json in search_twetter:
    #if each_json.full_text.startswith('RT'): #El if elimina los duplicados pero local, el mayor % de datos son RT
        #continue
...
with open('twetter_json_Data.txt', encoding= 'utf-8') as json_file:
    ...
    for each_diccionary in all_data:
        ...
        print(each_diccionary['user']['name'])
    ...
    #formila trae todo lo que está en user. No filtra el dato especifico que se requiere.
        ...
        twette_dataset = pd.DataFrame (my_demo_list, columns = ['created_at','tweet_id' , 'full_text','screen_name',
